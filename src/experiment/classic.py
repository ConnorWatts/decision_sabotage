...
from ..models.helper import HelperModel
...
class SingleClassicExperiment:
    """Runs single-participant experiment with AI 'helper' and participant models.

    Args:
        experiment_config (Dict): Configuration for experiment parameters
        helper (HelperModel): AI assistant model
        participant (ParticipantModel): AI participant model
        scenario (Scenario): Experiment scenario (business decision/document/context etc)
        time_limit (int, optional): Maximum duration in seconds. Defaults to 1800.
    """
    ...
    def __init__(
        ...
        self.logger = get_experiment_logger("classic_experiment")
        self.logger.info(f"Initializing ClassicExperiment with config: {self.config}")
        self.helper_model = helper
    ...
    async def run(self) -> ExperimentResult:
        ...
        try:
            # Initial scenario review period (not really needed for LLMs)
            ...
            for _ in range(self.num_interactions):
                ...
                # Get helper response
                helper_response = await self.helper_model.assist_with_decision(
                    scenario=self.scenario,
                    question=question,
                ...
                helper_responses.append(helper_response)
